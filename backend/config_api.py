...
def _validate_and_normalize_broker_config(payload: dict):
    if not isinstance(payload, dict):
        raise HTTPException(status_code=400, detail='expected JSON object')

    mode = str(payload.get('mode', 'internal')).strip().lower()
    if mode not in ('internal', 'external'):
        raise HTTPException(status_code=400, detail='mode must be "internal" or "external"')

    host = str(payload.get('host', '')).strip()
    if not host:
        raise HTTPException(status_code=400, detail='host is required')

    def _parse_port(name: str, default: int):
        ...
        if value < 1 or value > 65535:
            raise HTTPException(status_code=400, detail=f'{name} must be in 1..65535')
        return value

    tcp_port = _parse_port('tcp_port', 1883)
    ws_port = _parse_port('ws_port', 1884)
    ...
    return {
        ...
        'notes': notes if isinstance(notes, str) and notes.strip() else (
            'mode: internal => spawn local Aedes; external => do not spawn '
            '(assume external broker at host:tcp_port/ws_port)'
        ),
    }
...
def update_config(payload: dict):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f'failed to write broker config: {e}')
