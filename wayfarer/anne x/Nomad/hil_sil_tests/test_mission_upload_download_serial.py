...
def test_mission_upload_download_serial():
    test_name = "test_mission_upload_download_serial"
    start = time.time()
    ...
    conn = None
    try:
        conn = mavutil.mavlink_connection(PIXHAWK_DEVICE, baud=PIXHAWK_BAUD)

        # wait for vehicle heartbeat so we know the sysid/comp to target
        hb = conn.wait_heartbeat(timeout=10)
        if hb is None:
            duration = time.time() - start
            reporter.write_report(test_name, "FAIL", details="no heartbeat seen on serial", duration=duration)
            pytest.fail("No heartbeat from vehicle on serial port")

        target_sys = hb.get_srcSystem()
        target_comp = hb.get_srcComponent()

        # send a couple GCS heartbeats to make ourselves visible
        try:
            for _ in range(3):
